...
- FEAT: Outbound credentials live on the Account record (URL, user, password, custom fields) instead of environment variables — every outbound plugin reads them via the unified account lookup, so external secret stores (KeePass / Vault) work transparently
...
- FEAT: License soft policy — an expired license keeps every feature active and surfaces a warning instead of locking customers out. Optional `max_hosts` cap is a soft limit too: exceeding it shows an "over limit" badge but never disables anything. The license issuer's `--max-hosts` flag (0 = unlimited) emits the cap into the JWT
