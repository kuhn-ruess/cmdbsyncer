...
- SEC: ODBC plugin debug logging no longer leaks credentials — the account config dict and ODBC connection string are now masked (`password` / `PWD=***`) before being written to the debug log

## Version 3.12.11

- SEC: ODBC/MSSQL connections no longer silently disable TLS certificate validation — `TrustServerCertificate=YES` is now opt-in via the new `trust_server_certificate` account field (default: off)
