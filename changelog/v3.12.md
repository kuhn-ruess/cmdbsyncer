...
## Unreleased
- FIX: Expanded changelog sections on the admin index show their bullet list again — the previous release wrapped the heading inside `<summary>` with a nested `<h2>`, which some browsers/admin themes rendered so the body below was hidden. The content now lives in its own body div so every expanded version shows the entries that belong to it
