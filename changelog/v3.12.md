...
- SEC: API HTTPS gate no longer trusts `X-Forwarded-Proto` directly; added `TRUSTED_PROXIES` config (default 0) that enables werkzeug's ProxyFix only when explicitly set — prevents header-spoofing bypass of the HTTPS requirement for password-based API auth
