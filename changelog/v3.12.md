...
- SEC: HTML-escape changelog text in the admin index before rendering with `\|safe` — protects against XSS via a tampered `changelog.md` file
- SEC: Re-enable CSRF protection for the File Admin — the inline delete forms now emit the Flask-WTF `csrf_token` via a template fallback, so the global `csrf.exempt` on the File Admin blueprint is no longer needed
