...
class Host(HostLabelsMixin, db.Document):
    """
    Host
    """
    ...
    def get_host(hostname, create=True):
        ...
        """
        # Check the type first: a source handing over None or a number must
        # reach the caller as HostError, not as AttributeError from .strip().
    ...
    def set_inventory_attribute(self, key, value):
        ...
        Saves only when the value actually changed — the inventorize plugins
        call this per attribute in a loop, so an unconditional write costs one
        database roundtrip per unchanged attribute.

        Returns:
            bool: True if the attribute changed and was saved.
        """
