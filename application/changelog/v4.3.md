...
## Unreleased
- FEAT: In CMDB mode the host list gains a "Bulk add hosts" button (paste a plain hostname list or a CSV, optionally pick a template) and an "Import CSV" button next to Export CSV — the upload opens a preview where you pick which column is the hostname (any header works, not just `hostname`), toggle "first row has no header" if you forgot it, assign an optional template, and see the parsed rows before anything is created
