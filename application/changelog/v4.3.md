...
## Unreleased
- FEAT: When editing a host you can now pick "cmdb (CMDB managed)" in the Account field to hand the host over to the CMDB; this also clears its no-autodelete protection so maintenance can still clean it up if it goes stale
...
- FEAT: The "Check for valid hostname" and "Require FQDN" import checks are now configured per Account (Object Settings) instead of globally, and no longer apply to object accounts; if the old global CHECK_FOR_VALID_HOSTNAME / REQUIRE_FQDN settings are still in local_config.py, `sys self_configure` now warns you to migrate them to the accounts
