...
## Unreleased
- FEAT: In CMDB mode the host list gains a "Bulk add hosts" button (paste a plain hostname list or a CSV, optionally pick a template) and an "Import CSV" button next to Export CSV that reads the exported CSV format back in with the same template option
