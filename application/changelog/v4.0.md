...
## Unreleased
- FEAT: Host list quick-search now also matches inventory values, so searching for a serial / firmware / OS string finds the host even when the value lives in `inventory` instead of `labels`
- FEAT: Host list — click the small filter icon next to any template badge in a host row to group all hosts that share that template. Picks up the assigned `cmdb_templates` reference and filters the host list by it, the simplest way to group hosts by their CMDB template
- FIX: Host list row icons (clone/copy, history, debug) now have hover tooltips so it's clear what each icon does without clicking it
- FEAT: Host Debug page can preview a Checkmk Setup Rule's outcomes against the selected host — pick a `CheckmkRuleMngmt` entry from the new dropdown and the page shows the Jinja-rendered ruleset, folder, value and condition fields side-by-side with their templates, exactly the way `checkmk export_rules` would emit them. Loop outcomes (`loop_over_list`) expand into one preview row per loop value, and Jinja errors render as inline messages instead of breaking the page
...
- FEAT: CMDB template values support Jinja — fields on a CMDB template can reference the host's labels, inventory and `HOSTNAME` (e.g. `Server {{ HOSTNAME }} for {{ environment }}`). Rendering happens when template labels are merged into the host attributes; templates without `{{ ... }}` are passed through unchanged
