...
### VMware
- FEAT: VMware inventory now collects the resource pool, readable ESXi host name, VMware Tools version, network-card and disk counts, and the list of virtual disks, network cards and IDE/SCSI controllers per VM
- FEAT: An `inventory_filter` account setting limits custom-attribute sync and inventory to VMs matching any collected attribute, e.g. `power_state:poweredOn` or `power_state:poweredOn,guest_os:Linux` (same key repeated = OR, different keys = AND); new accounts default to `power_state:poweredOn`, clear it to process all VMs
...
### Hosts
- FEAT: Host list now marks Checkmk-sync-cache labels with a small cloud icon and a "?" help-tooltip on the Labels column header explaining the green vs. blue badges and why the green ones briefly disappear after editing a host
...
- FIX: The Archive now lists soft-deleted CMDB objects and templates as well, not only deleted hosts — they were previously hidden by an `is_object__ne=True` filter and effectively unreachable; a new Type column and filter let you narrow down again when needed
- FEAT: Host and Object list search box accepts boolean expressions: `AND`, `OR`, `NOT` (or `!`) and parentheses, plus optional `field:value` pairs (e.g. `(prod OR stage) AND NOT test`, `hostname:web*`, `env:prod`). Bare terms match the hostname (substring), the label / inventory keys (exact — use `*` for a prefix match) and the label / inventory values (substring), so `NOT basti_test` returns hosts without a label literally named `basti_test`. Quote values to include spaces or literal `*`
- FEAT: Host list has a one-click "Export CSV" button next to the saved-search bar; the export now honours the currently active filters and search expression instead of always dumping every host
