...
### Hosts
- FEAT: New "CMDB Tree" tab on the host detail page shows the full raw inventory tree per source, separate from the curated rule-engine inventory, with an "added / removed / changed since last import" diff banner
...
### Checkmk
- FEAT: HW/SW Inventorize now stores the full Checkmk inventory tree per host under Host → CMDB Tree; only the configured paths are still promoted to Host.inventory for the rule engine
