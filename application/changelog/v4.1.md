...
- FEAT: New `make release-pre` target ships sequential pre-release builds (`.devN` / `aN` / `bN` / `rcN`) to PyPI without requiring a hand-edit before each upload; `tools/build_offline_bundle.sh --pre` lets the offline bundle pick those pre-releases up so QA can test them on air-gapped systems
