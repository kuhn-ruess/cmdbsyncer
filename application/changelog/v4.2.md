...
### Interface
- FIX: Nested (subdocument) form fields — e.g. a rule's outcomes or conditions — are now theme-aware; their hardcoded light background no longer shows up white on the dark themes (Gruvbox, Dracula, Nord). `bg-light`/`bg-white` now follow the active theme everywhere
