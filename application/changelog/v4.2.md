...
- FEAT: Clicking a rule project opens a project page that lists its assigned Setup Rules and puts the folder import and the test/approve/prod actions right on that page
...
### Setup
- FEAT: The offline bundle builder can package cmdbsyncer from the current local git checkout with `--syncer-from-git`, to ship an unreleased state to air-gapped systems instead of downloading a published release from PyPI
