...
- FIX: The label history behind the host "Timeline" tab is no longer recorded by default (`LABEL_HISTORY_ENABLED` under Config → Local Config turns it back on) — it wrote one entry per changed label and nothing ever removed them, so on installations whose import rewrites labels on every run it had grown into the largest collection of the database by far
- FIX: The hostname filter of the host list takes long regex patterns again and says why a pattern was rejected instead of showing an empty list

### Rules
- FIX: Regex conditions take long patterns again — a list of 32 hostnames joined with | was silently rejected and matched nothing
...
- FIX: The Checkmk export no longer writes the folder attributes a second time for folders it just created in the same run
...
- FIX: An export names the host and the real error when a rule fails for it, instead of reporting every such host as "Timeout error"
...
### Hosts
- FIX: The "Assign Project" bulk action in the host list opens a dialog in the list (like "Set Template") instead of jumping to a separate page
