...
- FIX: A host that comes back after a while without a sighting is judged again instead of keeping the verdict it was given while it was missing — a rule switching a host off after so many days kept sending that state to the target system even though the host was back; only the rules that read the sighting date are recalculated, everything else stays as fast as it was
