...
class TestSeenTimestampDependency(unittest.TestCase):
    """Only a rule that reads an import timestamp flags its cache slot."""

    OFFLINE_PARAM = ("{{ 'offline' if syncer_last_seen is defined and "
                     "(datetime.datetime.utcnow() - syncer_last_seen).days "
                     "> 2 else 'prod' }}")
    ...
    def test_rendered_outcome_counts(self):
        self.rule.rules = [self._rule_doc(
            [{'action': 'set_criticality', 'param': self.OFFLINE_PARAM}])]
        self.assertTrue(self.rule.depends_on_seen_timestamps())

    def test_matched_condition_counts(self):
        # A rule can just as well match *on* the timestamp instead of
        # rendering it.
    ...
    def test_a_rule_without_the_timestamp_does_not(self):
        self.rule.rules = [self._rule_doc(
            [{'action': 'set_criticality', 'param': 'prod'}])]
        self.assertFalse(self.rule.depends_on_seen_timestamps())

    def test_answer_is_cached_with_the_rule_documents(self):
        doc = self._rule_doc(
            [{'action': 'set_criticality', 'param': self.OFFLINE_PARAM}])
        self.rule.rules = [doc]
        self.assertTrue(self.rule.depends_on_seen_timestamps())
    ...
    def test_get_outcomes_registers_the_slot(self):
        self.rule.rules = [self._rule_doc(
            [{'action': 'set_criticality', 'param': self.OFFLINE_PARAM}])]
        self.rule.get_outcomes(self.db_host, {})
        self.db_host.set_cache_slot_seen_dependent.assert_called_once_with(
            '_RuleForTests', True)

    def test_get_outcomes_deregisters_a_slot_that_no_longer_reads_it(self):
        self.rule.rules = [self._rule_doc(
            [{'action': 'set_criticality', 'param': 'prod'}])]
        self.rule.get_outcomes(self.db_host, {})
        self.db_host.set_cache_slot_seen_dependent.assert_called_once_with(
