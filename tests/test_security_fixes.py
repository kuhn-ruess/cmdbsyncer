...
reintroduces it fails here instead of in production. The XSS fix for the
host log column is covered in test_api.HostViewFormattingTest, next to
the other renderer escaping tests.
...
sql = _load_source('application.helpers.sql', 'application/helpers/sql.py')
get_account = _load_source(
    'application.helpers.get_account', 'application/helpers/get_account.py')


class CustomQueryValidationTest(unittest.TestCase):
    """
    The keyword checks run against the comment-stripped statement, so the
    statement handed on for execution must be the stripped one. MySQL
    executes conditional comments (/*!50000 ... */) as real SQL.
    """

    def test_returns_the_validated_statement(self):
        self.assertEqual(
            sql.validate_custom_query('SELECT 1 /* just a note */'), 'SELECT 1')

    def test_mysql_conditional_comment_cannot_smuggle_ddl(self):
        result = sql.validate_custom_query(
            'SELECT id FROM hosts /*!50000 DROP TABLE hosts*/')
        self.assertNotIn('DROP', result.upper())

    def test_conditional_comment_stripped_with_allow_ddl(self):
        result = sql.validate_custom_query(
            'CREATE TABLE IF NOT EXISTS t (id INT); SELECT id FROM t '
            '/*!50000 DROP TABLE hosts*/',
            allow_ddl=True)
        self.assertNotIn('DROP', result.upper())

    def test_plain_write_statement_still_rejected(self):
        with self.assertRaises(ValueError):
            sql.validate_custom_query('DELETE FROM hosts')
